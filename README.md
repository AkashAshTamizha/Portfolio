...
cd backend
npm install
cp .env.example .env      # fill in MONGO_URI, SMTP creds (optional), and Cloudinary creds
...
**Uploads (images, resumes/certificates, project videos)** are stored on [Cloudinary](https://cloudinary.com), not on local disk. Create a free Cloudinary account, then from the dashboard set either `CLOUDINARY_URL` or the three discrete vars (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`) in `backend/.env`. Without these set, calls to `/api/upload/*` will fail — everything else in the app works fine regardless.
...
4. Add environment variables from `.env.example`: `MONGO_URI`, `CLIENT_ORIGIN` (your Vercel URL), `SMTP_*`, `CONTACT_RECEIVER_EMAIL`, `CLOUDINARY_*`.
...
2. From the dashboard, copy either the single `CLOUDINARY_URL` connection string, or the three separate values (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`).
3. Add whichever you chose to the backend's environment variables (Render, Railway, etc.) and to your local `.env`.
4. All uploads (`POST /api/upload/image`, `POST /api/upload/file`) — profile photos, project images, certification files, resumes, and short project videos — are streamed directly to Cloudinary and organized under `<CLOUDINARY_FOLDER>/<type>/`, e.g. `portfolio/projects/...`.
